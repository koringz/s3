    ...
    !function (factory) {
        ...
        */
            ...
            var category = [
                {},
                {
                    ...
                    circle: function () {
                        ...
                        this._staticFingerprints = new Map();
                        this._dynamicFingerprints = new Map();
                    },
                    line: function () {
                        this.name = 'line';
                        this.data = [];
        ...
        body.prototype.set = function (options) {
            var params = [options];
            var results = this.mounts.set(this.dist[0]);
        ...
        (function () {
            ...
            Object.assign(s3.category[1][TOOL_s].prototype, {
                ...
                amount: function (params) {
                    var amountObj = new s3.category[1][AMOUNT_s](params);
                    var vals = amountObj.value;
                    return amountObj.set(vals)
                },
                manager: function () {
                    var that = this;
                    this.pipe = [];
                    ...
                    return {
                        ...
                        graphics: {
                            ...
                            addComponent: function () {
                                ...
                                if (_s3.isStr(shapeName) && _s3.isObj(getCharacteristic)) {
                                    that.pipe.push({
                                        componentName: shapeName,
                                        characteristic: getCharacteristic
                                    });
                                }
                                var getPropBindObject;
                                try {
                                    if (s3.category[1][shapeName]?.prototype?._constructor === shapeName) {
                                        getPropBindObject = shapeName;
                                    }
                                } catch (e) {
        ...
        (function () {
            ...
            Object.assign(s3.category[1][CIRCLE_s].prototype, {
                ...
                execute2DEngine: function () {
                    ...
                    var motionControl = arguments[0][2];
                    this.o_OPEN = motionControl.open;
                    
                    if (this.o_OPEN)
                        this.s_SPEED = motionControl.speed;
                ...
                draw: function (ctx, getShapeProperty) {
                    var cw = this.canvasNode.canvas.offsetWidth;
                    var ch = this.canvasNode.canvas.offsetHeight;

                    // 获得2d 原型方法
                    this.ctx2d = new ctx(this.canvasNode);
                    ...
                    // 调用状态
                    // _updateDynamicParticle 循环方法 
                    // 内部循环方法 changedJsData
                    ...
                    this._initStaticAndDynamic(props.property, shapeProperty);

                    this._renderstaticCache(props.property, shapeProperty, cw, ch)
                    this._renderDynamicLayer(props.property, shapeProperty)
                    ...
                    return {
                        ...
                        reloadAnimation: function () {
                            ...
                            that._renderstaticCache(props.property, shapeProperty, cw, ch)
                            that._renderDynamicLayer(props.property, shapeProperty)
                        }
                    }
                },
                // ===== 每帧入口 =====
                ...
                // ===== 指纹粒度 =====
                _initStaticAndDynamic: function (particle, shapeProperty) {
                    for (var j = 0, len = particle.length; j < len; j++) {
                        particle[j].key = j || particle[j]?.key;
                        this.mergeIterator(particle[j], shapeProperty);

                        const _fp = this._computeFingerprint(this.iterator);
                        if (!this.iterator.motion || this.iterator.motion === 'stop') {
                            this._staticFingerprints.set(particle[j]?.key, _fp);
                        } else if (this.iterator.motion === 'move') {
                            this._dynamicFingerprints.set(particle[j]?.key, _fp);
                        }
                    }
                },
                ...
                // ===== 启动渲染 =====
                _renderstaticCache: function (particle, shapeProperty, cw, ch) {
                    // 1) 静态层：指纹检测变化 → 变化才重建 → 每帧 drawImage
                    this._syncStaticCache(particle, shapeProperty, cw, ch);
                    if (this.staticCache) {
                        this.ctx2d.hook2D('drawImage', {
                            image: this.staticCache,
                            x: 0,
                            y: 0
                        });
                    }
                ...
                // ===== 静态层：指纹检测变化 + 重建 + 缓存 =====
                _syncStaticCache: function (particle, shapeProperty, cw, ch) {
                    var dirty = false;

                    const staticParticles = particle.filter(item => !item?.motion || item?.motion === 'stop');
                    
                    // 尺寸变化 → 重建
                    if (!this.staticCache
                        || this.staticCache.width !== cw
                        || this.staticCache.height !== ch) {
                        dirty = true;
                    }
                    // 粒子数量变化 → 重建
                    // 要注释- 因为数量不变，值可能改变
                    // else if (staticParticles.length !== this._staticFingerprints.size) {
                    //     dirty = true;
                    // }
                    // 逐个粒子指纹比对 → 任何变化都重建
                    else {
                        for (var i = 0; i < particle.length; i++) {
                            if (!particle[i]?.motion || particle[i]?.motion === 'stop') {
                                ...
                                var fp = this._computeFingerprint(this.iterator);
                                if (this._staticFingerprints.get(_KEYS) !== fp) {
                                    dirty = true;
                                    break;
                                }
                            }
                    ...
                    this.staticCache.width  = cw;
                    this.staticCache.height = ch;
                    var sctx = this.staticCache.getContext('2d');
                    sctx.clearRect(0, 0, cw, ch);

                    var groups = {};
                    this._staticFingerprints.clear();
                    for (var i = 0; i < particle.length; i++) {
                        var _KEYS = i || particle[i]?.key;
                        if (!particle[i]?.motion || particle[i]?.motion === 'stop') {
                            this.mergeIterator(particle[i], shapeProperty);
                            this._addStaticPath(groups, shapeProperty);
                            this._staticFingerprints.set(_KEYS, this._computeFingerprint(this.iterator));
                        }
                    }
                    for (var c in groups) {
                        sctx.fillStyle = c;
                ...
                _computeFingerprint: function (it) {
                    return  it.x + '|' + it.y + '|' + it.color + '|' + it.radius + '|' + 
                            it.startAngles + '|' + it.stopAngles + '|' + it.speed + '|' + it.trails + '|' + 
                            it.motion + '|' + it.bMoveMode + '|' + it.x1+ '|' + it.y1;
                },

                _addStaticPath: function (groups, shapeProperty) {
                ...
                // ==============================  动态部分 ==============================
                _renderDynamicLayer: function (particle, shapeProperty) {
                    var dynamicGroups = {};
                    ...
                    for (var j = 0, len = particle.length; j < len; j++) {
                        ...
                        if (this.iterator.motion === 'move') {
                            if(this._dynamicFingerprints.get(particle[j].key) !== JSON.stringify(this.iterator)){
                                this._dynamicFingerprints.set(particle[j].key, JSON.stringify(this.iterator));
                            }
                            ...
                            // motion = move, 需要运动
                            // 每一帧调用loop方法, 更新粒子的位置
                            this._updateDynamicParticle(j, dynamicGroups);
                        }
                    }

                    // 填充空间范围
                    this._fillPathGroups(dynamicGroups);
                },
                _updateDynamicParticle: function (j, groups) {
                    ...
                    // this._buildPath(results, groups);
                    // this._syncParticleState(j);
                    // this.hasMoving = true;
                    /**
                    ...
                    try {
                        ...
                        // 信号1 - 0帧 - 提供基础数据和器材 - 信号是介质，如声音 信号一样可以内外联通的信息
                        var results = this.invokeExternalMethodWithDataSource();
                ...
                // 信号1 - 0帧 - 信号是介质，如声音 信号一样可以内外联通的信息
                // external - 外部 method 接收传参 exposeDataSource
                ...
                // 结束 - 静止中
                _fillPathGroups: function (groups) {
                ...
                mergeIterator: function (particle, shapeProperties) {
                    ...
                    var p = particle;
                    var s = shapeProperties;
                    var pi = s.pi;
                    
                    // 直接赋值，无任何额外开销
                    it.key = p.key !== undefined ? p.key : s.key;
                    it.x = p.x !== undefined ? p.x : s.x;
                    it.y = p.y !== undefined ? p.y : s.y;
                    it.color = p.color !== undefined ? p.color : s.color;
                    it.speed = p.speed !== undefined ? p.speed : s.speed;
                    it.radius = p.radius !== undefined ? p.radius : (s.radius || 36);
                    it.trails = p.trails !== undefined ? p.trails : s.trails;
                    it.startAngles = (p.startAngles !== undefined ? p.startAngles : 0) * pi;
                    it.stopAngles = (p.stopAngles !== undefined ? p.stopAngles : Math.PI * 2) * pi;
                    it.motion = p.motion !== undefined ? p.motion : s.motion;
                    it.bMoveMode = p.bMoveMode !== undefined ? p.bMoveMode : s.bMoveMode;
                    it.method = p.method !== undefined ? p.method : s.method;
                    it.x1 = p.x1 !== undefined ? p.x1 : s.x1;
                    it.y1 = p.y1 !== undefined ? p.y1 : s.y1;
                    it.custom = p.custom !== undefined ? p.custom : s.custom;
                },
