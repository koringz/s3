       ...
       (function () {
            ...
            Object.assign(s3.category[1][CIRCLE_s].prototype, {
                ...
                draw: function (ctx, getShapeProperty) {
                    ...
                    this._initStops(props.property, shapeProperty)
                    this._initMoves(props.property, shapeProperty)

                    this._renderStop(props.property, shapeProperty)
                ...
                // ===== 指纹粒度 =====
                _initStops: function (particle, shapeProperty) {
                    for (var j = 0, len = particle.length; j < len; j++) {
                        particle[j].key = j || particle[j]?.key;
                        const getMerges = this.mergeProperties(particle[j], shapeProperty);
                        Object.assign(this.iterator, getMerges);

                        if (!this.iterator.motion || this.iterator.motion === 'stop') {
                            this._stopFp.set(particle[j]?.key, JSON.stringify(getMerges));
                        }
                    }

                },
                _initMoves: function (particle, shapeProperty) {
                    for (var j = 0, len = particle.length; j < len; j++) {
                        particle[j].key = j || particle[j]?.key;
                        const getMerges = this.mergeProperties(particle[j], shapeProperty);
                        Object.assign(this.iterator, getMerges);
                        ...
                        if (this.iterator.motion === 'move') {
                            this._moveFp.set(particle[j]?.key, JSON.stringify(getMerges));
                        }
                    }
                ...
                // ===== 启动渲染 =====
                _renderStop: function (particle, shapeProperty) {
                    var staticGroups  = {};
                    // this.hasMoving = false;
                    // this.hasStatic = false;
                    // this.noMotionArrCount = [];
                    ...
                    for (var j = 0, len = particle.length; j < len; j++) {
                        ...
                        if (!this.iterator.motion || this.iterator.motion === 'stop') {
                            ...
                            // motion = stop 或 为空
                            // 说明粒子是静态的，不需要运动
                            this._pushPath(staticGroups, shapeProperty);
                            // this.hasStatic = true;
                            // this.noMotionArrCount.push(j);

                            this._stopFp.set(particle[j]?.key, getMerges);
                        }
                    }

                    this._fillGroups(staticGroups);
                },
                _renderMove: function (particle, shapeProperty) {
                    ...
                    for (var j = 0, len = particle.length; j < len; j++) {
                        ...
                        if (this.iterator.motion === 'move') {
                            if(this._moveFp.get(particle[j].key) !== getMerges){
                                this._moveFp.set(particle[j].key, getMerges);
                            }
                ...
                mergeProperties: function (particle, shapeProperties) {
